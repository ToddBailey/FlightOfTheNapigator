...
// NOTE:  if there is not an ISR-based human-clock, we can get rid of the cli() and SREG stuff here since we don't have to worry about atomicity.
// NOTE:  Timer ticks are sometimes 16-bit variables and sometimes longs in different apps.  The longest we can time will obviously change depending on this.

#include "includes.h"


static unsigned long			// Local variables which keep track of timer stuff. 
